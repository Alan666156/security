...
public class PDFUtil {
	...
	public static final String PASSWORD = "3SDEcowEjM1=";
	private static final int QM_IMG_WIDTH = 160; // 二维码宽度
	private static final int QM_IMG_HEIGHT = 160; // 二维码高度
	 ...
	 * theCell.setBorder(Rectangle.NO_BORDER); theCell.addElement(new Paragraph(
	 * "                   " + customer.getPostCode(), envelopeFont)); String[]
	 * infos = customer.getAdress().split("\\|"); for (int i = 0; i <
	 * infos.length; i++) { theCell.addElement(new Paragraph(
	 * "                   " + infos[i], envelopeFont)); }
	 * 
	 * String gender = "";
	 * 
	...
	public static Paragraph getHoriCell() throws DocumentException, IOException {
		String horizontally = "   ";
	 ...
	 * @throws Exception
	 */
	public static byte[] insertQRCodeToPdf(String qrContent, String tipTxt, int margin_x, int margin_y, byte[] inPdfBytes) throws Exception {
		byte[] data = null;
		float tip_margin_width = 5f;
		float tip_margin_height = 15f;
		PdfReader pdfReader = new PdfReader(inPdfBytes);
		ByteArrayOutputStream bOutStream = new ByteArrayOutputStream();
		PdfStamper pdfStamper = new PdfStamper(pdfReader, bOutStream);
		PdfContentByte content = pdfStamper.getOverContent(1);
		// 插入二维码
		float contentWeidth = content.getPdfDocument().getPageSize().getWidth();
		float contentHeight = content.getPdfDocument().getPageSize().getHeight();
		LOGGER.info("添加二维码操作：pdf首页contentWeidth=" + contentWeidth + ", contentHeight=" + contentHeight);
		byte[] qrArray = QuickMarkUtil.encodeToByteArray(qrContent, QM_IMG_WIDTH, QM_IMG_HEIGHT);
		if (qrArray == null || qrArray.length == 0) {
			throw new Exception("生成二维码异常");
		}
		Image img = Image.getInstance(qrArray);// 选择图片
		img.setAlignment(1);
		img.scaleAbsolute(100f, 100f); // 设置比例
		img.setAbsolutePosition(contentWeidth - margin_x, contentHeight - margin_y);
		content.addImage(img);
		// 增加提示文本
		content.beginText();
		content.setFontAndSize(BaseFont.createFont("STSong-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED), 9);
		content.setTextMatrix(contentWeidth - margin_x + tip_margin_width, contentHeight - margin_y - tip_margin_height);
		content.showText(tipTxt);
		content.endText();
		content.stroke();
		pdfStamper.close();
