...
/**
 * 双向链表+HashMap来实现(基本实现思路：利用HashMap以及双端队列来实现，利用Map来保存key与Node节点的对应关系，实现O(1)查询操作；利用双端队列来保存Node节点链表，当查询命中时，将Node节点移动到链表头部；当插入时，如果容量未满，则直接插入到链表头部，如果容量已满则删除链表最后元素，将新元素插入到链表头部，这样链表头部一直保存最新的Node节点，尾部保存最老的Node节点)
 *
